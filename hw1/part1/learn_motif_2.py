...
def detect_threshold(old_prob_martrix,new_prob_matrix,threshold):
    test_old = old_prob_martrix[:,1:]
    test_new = new_prob_matrix[:,1:]
    diff_matrix = test_old - test_new
    ratio = diff_matrix/test_new
    ratio_abs = np.absolute(ratio)
    minNum = np.amin(ratio_abs)
    if minNum < threshold:
        return True
    else:
        return False
...
def EM(input_matrix,L,W,N,threshold):
    seed_list = []
    likelihood_list = []
    prob_matrix = generate_Prob_Matrix(W, input_matrix, 0)
    for iter in range(1000):
        print(iter)
        cur_seed = iter
        seed_list.append(cur_seed)
        prob_matrix = generate_Prob_Matrix(W,input_matrix,cur_seed)
        ...
        z_matrix = get_updated_Z_matrix(input_matrix, prob_matrix, L, W, N)
        start_list = findStartPosition(z_matrix)
    ...
    while(detect_threshold(old_p,new_p,threshold) == False):
        z_matrix = get_updated_Z_matrix(input_matrix, new_p, L, W, N)
        old_p = copy.deepcopy(new_p)
        new_p = get_updated_prob_matrix(z_matrix, input_matrix, prob_matrix, L, W, N)
        # count += 1
        # print('loop ',count)
    return new_p, z_matrix, best_seed
...
def main(args):
    ...
    final_p_matrix, final_z_matrix, best_seed = EM(input_matrix, L, W, N, 1e-6)
