...
def generate_Prob_Matrix(W,subsequence):
    prob_matrix = np.zeros((4,W+1))
    row,col = prob_matrix.shape
    for i in range(row):
        for j in range(col):
            prob_matrix[i][j] = 0.25
    sub_list = list(subsequence)
    for i in range(4):
        for j in range(1,col):
            if int(sub_list[j-1]) == i:
                prob_matrix[i][j] = 0.88
            else:
                prob_matrix[i][j] = 0.04
    return prob_matrix
...
def generate_initial_subsequence(input_matrix,z_matrix,N,W,L):
    x,y = z_matrix.shape
    subsequence_matrix = []
    for i in range(x):
        subsequence_row = []
        for j in range(y):
            subsequence = ''
            for k in range(W):
                subsequence = subsequence + str(input_matrix[i,j+k])
            subsequence_row.append(subsequence)
        subsequence_matrix.append(subsequence_row)
    return subsequence_matrix
...
def cal_loglikelihood(start_list,input_matrix,prob_matrix,W,L,N):
    ...
    for i in range(N):
        ...
        for k in range(start_pos):
            ...
            log_likelihood += math.log10(cur_prob)
        for j in range(W):
            pos = start_pos + j
            letter = Xi[pos]
...
def EM(input_matrix,L,W,N,threshold,seed,subsequence):

    prob_matrix = generate_Prob_Matrix(W,subsequence)
    z_matrix = get_updated_Z_matrix(input_matrix, prob_matrix, L, W, N)

    old_z_matrix = copy.deepcopy(z_matrix)
    old_p = copy.deepcopy(prob_matrix)

    new_p = get_updated_prob_matrix(old_z_matrix,input_matrix,old_p,L,W,N)
    new_z_matrix = get_updated_Z_matrix(input_matrix,new_p,L,W,N)
...
def main(args):
    ...
    z_matrix = generate_Z_Matrix(N,W,L,666)
    subsequence_matrix = generate_initial_subsequence(input_matrix,z_matrix,N,W,L)

    subsequence_list = []
    likelihood_list = []
    index_list = []
    for seq in range(len(subsequence_matrix)):
        for start in range(len(subsequence_matrix[0])):
            subsequence = subsequence_matrix[seq][start]
            if subsequence in subsequence_list:
                index = subsequence.index(subsequence)
                subsequence_list.append(subsequence)
                likelihood = likelihood_list[index]
                likelihood_list.append(likelihood)
                tup = index_list[index]
                index_list.append(tup)
            else:
                subsequence_list.append(subsequence)
                prob_matrix = generate_Prob_Matrix(W,subsequence)
                z_matrix = get_updated_Z_matrix(input_matrix, prob_matrix, L, W, N)
                start_list = get_start_list(z_matrix)
                prob_matrix = get_updated_prob_matrix(z_matrix,input_matrix,prob_matrix,L,W,N)
                likelihood = cal_loglikelihood(start_list, input_matrix, prob_matrix, W, L, N)
                likelihood_list.append(likelihood)
                index_list.append((seq,start))
        print('done subsequence ', seq)
    likelihood_arr = np.array(likelihood_list)
    print(likelihood_arr)

    sort_index = np.argsort(likelihood_arr)
    best_seq,best_start = index_list[sort_index[-1]]
    print('best start sequence: ',best_seq)
    print('best start position: ',best_start)
    best_subsequence = subsequence_matrix[best_seq][best_start]

    final_p_matrix,final_z_matrix = EM(input_matrix, L, W, N, 0.0000001, 666, best_subsequence)
    # print(final_p)
